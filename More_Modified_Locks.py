...
def put_lists(queue, use_list, lock):
    for i in use_list:
        lock.acquire()
        queue.put(i)
        lock.release()
...
def addtosheet(full_queue,lock):
    lock.acquire()
...
def full_run():
    ...
    extend1 = multiprocessing.Process(target=put_lists, args=(full_queue, full_list, lock2))
    read1 = multiprocessing.Process(target=addtosheet, args=(full_queue, lock2))
##    put_in_queue(full_queue, ms1_list)
##    put_in_queue(full_queue, ms2_list)
##    put_in_queue(full_queue, ms3_list)
##    put_in_queue(full_queue, ms4_list)
